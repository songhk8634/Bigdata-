...
# 작업 디렉토리 확인 및 설정
getwd()
setwd('C:/Users/admin/Documents/R/r-base')

# 원본 데이터 읽기
data_cust <- read.csv('./Data/BGCON_CUST_DATA.csv', header = T, sep = ',',
                      encoding = 'CP949', fileEncoding = 'UCS-2')
data_claim <- read.csv('./Data/BGCON_CLAIM_DATA.csv', header = T, sep = ',',
                      encoding = 'CP949', fileEncoding = 'UCS-2')
data_cntt <- read.csv('./Data/BGCON_CNTT_DATA.csv', header = T, sep = ',',
                      encoding = 'CP949', fileEncoding = 'UCS-2')
data_fmly <- read.csv('./Data/BGCON_FMLY_DATA.csv', header = T, sep = ',',
                      encoding = 'CP949', fileEncoding = 'UCS-2')
data_fpinfo <- read.csv('./Data/BGCON_FPINFO_DATA.csv', header = T, sep = ',',
                      encoding = 'CP949', fileEncoding = 'UCS-2')
dim(data_cust)
dim(data_claim)
dim(data_cntt)
...
# 정상, 사기자 수 확인
(count_siu <- table(data_cust$SIU_CUST_YN)) # 사기자 수 : 1806, 정상 수 : 18801
...
unlist(data_cust$CTPR[1:4])  # list 안 list해제
